...
func (e *MachineService) GetMachine(orgId string) ([]*machine_model.Machine, error) {
...
// but not now
func (e *MachineService) RecordStatus(machineId string, status string) error {
	...
	err := e.redis.Set(context.Background(), fmt.Sprintf("machine:last_alive:%s", machineId), time.Now().Unix(), 10*time.Second).Err()
	if err != nil {
		logger.Error().Err(err).Str("machineId", machineId).Str("status", status).Msg("Failed to record status")
		return err
	}
	return nil
}
